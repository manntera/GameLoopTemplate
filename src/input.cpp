...
HRESULT UpdateKeyboard(void)
{
	...
	if(SUCCEEDED(hr))
	{
		...
		{
			// トリガーとリリースを取得
			g_keyStateTrigger[cnt] = g_keyState[cnt] & (g_keyState[cnt] ^ keyStateOld[cnt]);
			g_keyStateRelease[cnt] = keyStateOld[cnt] & (g_keyState[cnt] ^ keyStateOld[cnt]);
			
			// キーが押されているならリピートの判定処理
			if(g_keyState[cnt])
			{
				if(g_keyStateRepeatCnt[cnt] < 20)		//オーバーフロー回避
				{
					g_keyStateRepeatCnt[cnt]++;
					// 「初回入力」もしくは「ボタンを押してから20フレーム経過」
					// した場合、リピート用配列のデータを有効にする
					if(g_keyStateRepeatCnt[cnt]>=20 || g_keyStateRepeatCnt[cnt]==1)
					{
			...
			{
				g_keyStateRepeatCnt[cnt] = 0;	//カウントを0に
				g_keyStateRepeat[cnt] = 0;		//リピート判定フラグを0に
			}
		}
	}
	else
...
{
	return ((g_mouseState.rgbButtons[1] & 0x80) != 00);
}
